...
void instantiateLight(std::string name, glm::vec3 pos, glm::vec3 rot, glm::vec3 scale, std::vector<std::string> extraArgs) {
	if (name == "point") lights.emplace_back(new Light(pos, glm::vec3(1, 1, 1), extraArgs.size() > 0 ? std::stof(extraArgs[0]) : 200));
	else if (name == "Flicker") lights.emplace_back(new FlickerLight(pos, glm::vec3(1, 1, 1), extraArgs.size() > 0 ? std::stof(extraArgs[0]) : 200));
}
