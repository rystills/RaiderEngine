...
int main() {
	...
	addTextObject(new FpsDisplay(6, TARGET_HEIGHT - 20,glm::vec3(1,1,1), "Inter-Regular", 18));
	addTextObject(new TextObject("Press f3 to toggle physics wireframes",6, TARGET_HEIGHT - 50, glm::vec3(.8f, .2f, .5f), "Inter-Regular", 24));
	addTextObject(new TextObject("Use WASD to move, space to jump, left ctrl to crouch, and left shift to sprint", 6, TARGET_HEIGHT - 80, glm::vec3(.5f, .8f, .2f), "Inter-Regular", 24));
	addTextObject(new TextObject("Press left mouse to grab objects, and right mouse to observe hovered objects", 6, TARGET_HEIGHT - 110, glm::vec3(.2f, .5f, .8f), "Inter-Regular", 24));
	addTextObject(new TextObject("Press 1-3 to switch between the demo scenes", 6, TARGET_HEIGHT - 140, glm::vec3(1, 1, 0), "Inter-Regular", 24));
	TextObject* displayStringIndicator = addTextObject(new TextObject("", TARGET_WIDTH / 2.f, TARGET_HEIGHT / 2.f,glm::vec3(1.f),"Inter-Regular",24,true));
