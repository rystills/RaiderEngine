...
GameObject* ObjectRegistryBase::instantiateGameObject(std::string name, glm::vec3 pos, glm::vec3 rot, glm::vec3 scale, std::vector<std::string> extraArgs) {
	...
	else return addGameObject(new GameObject(pos, rot, scale, name, 0, true, true, mapNodeFlags.usePhysics, mapNodeFlags.castShadows));  // default; spawn a named model as a generic GameObject
	return NULL;
}
