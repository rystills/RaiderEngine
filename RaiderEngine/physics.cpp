...
void GEventCallback::onTrigger(PxTriggerPair* pairs, PxU32 count) {
	...
	for (int i = 0; i < count; ++i) {
		if (pairs[i].otherActor == PlayerSpawn::player->controller->getActor() && ((GameObject*)pairs[i].triggerActor->userData)->model->surfType == "ladder")
			PlayerSpawn::player->updateLadderVolumeCount(pairs[i].status == PxPairFlag::eNOTIFY_TOUCH_FOUND, (GameObject*)pairs[i].triggerActor->userData);
		else if (pairs[i].otherActor == PlayerSpawn::player->waterCheckBody && pairs[i].otherShape == PlayerSpawn::player->waterCheckShape && ((GameObject*)pairs[i].triggerActor->userData)->model->surfType == "water")
